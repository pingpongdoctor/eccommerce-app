...
export const NEW_PRODUCTS_QUERY = groq`*[_type == "post" && defined(slug) && defined(category) && count(images)>0] | order(_updatedAt desc)[0...4]`;
export const FEATURED_PRODUCTS_QUERY = groq`*[_type == "post" && defined(slug) && defined(category) && count(images)>0 && featured == $featured] | order(_updatedAt desc)[0...4]`;
